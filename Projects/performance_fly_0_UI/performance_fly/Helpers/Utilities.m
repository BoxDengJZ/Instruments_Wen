...
+ (nullable NSURLSessionTask *)downloadImageForURL:(NSURL *)url completion:(void (^)(UIImage *, NSURL *realURL))block
{
    ...
    if (image) {
        ...
        NSURLSessionDataTask *task = [session dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            if (data) {
                ...
                if (image) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [simpleImageCache setObject:image forKey:response.URL];
                        block(image, response.URL);
                    });
                }
            }
        }];
